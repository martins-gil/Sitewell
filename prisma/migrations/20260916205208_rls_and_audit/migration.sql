...
-- app_runtime is that role â€” it's what DATABASE_URL should point at.
-- -----------------------------------------------------------------------------
DO $$
BEGIN
...
--   app.current_org_id     â€” the caller's organization_id
--   app.is_platform_admin  â€” 'true' for cross-org Platform Admin support access
--
-- Looped via dynamic SQL instead of writing ~40 near-identical CREATE POLICY
-- statements by hand â€” same policy shape on every tenant table, so a typo in
...
-- audit_log: read-only for app_runtime (no write grants at all â€” see above),
...
-- trustworthy â€” no app code path (or bug) can edit or delete history.
-- -----------------------------------------------------------------------------
