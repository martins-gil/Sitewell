...
export function VisitKits({
  ...
  return (
    <div className="rounded-lg border border-neutral-200 dark:border-neutral-800">
      {kits.length === 0 ? (
        <p className="px-4 py-3 text-sm text-neutral-400">No kits assigned to this visit yet.</p>
      ) : (
          ...
          {kits.map((kit) => (
            <li key={kit.id} className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm">
              <span className={kit.used ? "opacity-60" : undefined}>
                ...
                <span className="text-neutral-500">· expires {kit.expiryLabel}</span>
                {kit.used && <span className="ml-2 text-xs text-neutral-500">(used — removed from inventory)</span>}
              </span>
              {!kit.used && (
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-neutral-200 px-4 py-3 dark:border-neutral-800">
        ...
        >
          <option value="">
            {availableKits.length === 0 ? "No unassigned kits for this study" : "Assign a kit from inventory…"}
          </option>
          {availableKits.map((k) => (
